...
import type { Destination, Trip, TripIdea } from '../lib/types.ts';
      ...
      <main className="tr-body">
        {!destinations.length ? (
          ...
          <>
            ...
            {canEdit && (adding ? (
              <DestinationForm group={activeGroup ?? 'Ziele'} onDone={() => setAdding(false)} />
            ) : (
      ...
      {canEdit && (
        <>
          ...
          {own
            ...
            : <p className="add-note"><button className="btn" onClick={() => setOwn(true)}>Eigenes Ziel</button></p>}
        </>
      )}
    </div>
  );
